...
function PlayMoanEx(Actor ActorRef, int Strength = 30, bool IsVictim = false, bool UseLipSync = false, int SoundCut = 0, float MoveTime = 0.2, int Phoneme = 1, int MinValue = 20, int MaxValue = 50, bool IsFixedValue = false, bool UseMFG = false)
	...
	if !UseLipSync
		...
		endIf
	elseIf LipSync && LipSync != Config.LipSync && Game.GetCameraState() != 3 && ((Strength > JsonUtil.GetIntValue(File, "sl_hot_voice_strength", 75) && HasTag("TopicHot")) || (IsVictim && HasTag("TopicMedium")) || HasTag("TopicMild") || !(HasTag("TopicMedium") || HasTag("TopicHot")))
